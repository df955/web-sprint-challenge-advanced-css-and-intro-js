...
function get20s(artists){
  ...
  for(let l = 0; l < artists.length; l++){
    let birthYear = parseInt(artists[l].years.slice(0,4))
    let deathYear = parseInt(artists[l].years.slice(6,11))
    if(birthYear>=1900 && birthYear<=2000 && deathYear>=1900 && deathYear<=2000){
      arr.push(artists[l].name);
    }
} return arr;
}
...
function lotsOfArt(artists){
 let paint100 = [];
 for(let l = 0; l < artists.length; l++){
  let paintNum = parseInt(artists[l].paintings)
   if( paintNum > 100){
     paint100.push(artists[l].name);
   }
 }
 return paint100;
}
